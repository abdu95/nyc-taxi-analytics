...
def ensure_gcs_bucket_exists() -> None:
    ...
    try:
        ...
        print(f"Bucket not found — creating gs://{GCS_BUCKET_NAME} ...")
        bucket = client.create_bucket(GCS_BUCKET_NAME, location=GCS_LOCATION)
...
def download_and_upload_to_gcs(taxi_type: str, year: int, month: int) -> None:
    ...
    if blob.exists():
        print(f"⚠️ File already exists in GCS, skipping upload: gs://{GCS_BUCKET_NAME}/{gcs_path}")
        return
...
def load_gcs_to_bigquery(taxi_type: str, year: int, month: int) -> None:
    ...
    try:
        ...
        if table.num_rows > 0:
            print(f"⚠️ Table already exists with {table.num_rows:,} rows, skipping load: {table_id}")
            return
    except NotFound:
